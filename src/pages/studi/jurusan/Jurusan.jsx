...
const Jurusan = () => {
   ...
   function handleDelete() {
      deleteData(`/jurusan/${selectToDelete?.id}`)
         .then((res) => {
            if (res.status === 200) {
               onClose();
               toast({
   ...
   function handleSubmit(e) {
      ...
      postData("/jurusan", formData)
         .then((res) => {
            if (res.status === 200) {
               toast({
                  status: "success",
                  description: "berhasil menambahkan jurusan/prodi",
               });
            }
         })
         .catch((err) => {
   ...
   return (
      <Chakra.Stack>
               ...
               <Chakra.ModalBody>
                  <form onSubmit={handleSubmit}>
                     <Chakra.Stack>
                        ...
                        <Chakra.FormControl isRequired>
                           <Chakra.FormLabel htmlFor="jenjang">
                              jenjang
                           </Chakra.FormLabel>
                           <Chakra.Input
                              onChange={handleChange}
                        ...
                        {fakultas.fetchIsLoading ? (
                           <Chakra.Skeleton h="10" />
                        ) : (
                           fakultas.data !== null && (
                              <Chakra.FormControl isRequired>
                                 <Chakra.FormLabel htmlFor="fakultas">
                                    Fakultas
                                 </Chakra.FormLabel>
                                 <Chakra.Select
                                    name="fakultas"
                                    id="fakultas"
                                    onChange={handleChange}
                                    defaultValue={"pilih fakultas"}>
                                    <option disabled value="pilih fakultas">
                                       pilih fakultas
                                    </option>
                                    {fakultas.data?.map((fk) => (
                                       <option value={fk.name} key={fk.id}>
                                          {fk.name}
                                       </option>
                                    ))}
                                 </Chakra.Select>
         ...
         <Chakra.TableContainer mt="4">
               ...
               <Chakra.Table variant="striped">
                  <Chakra.Thead>
                     <Chakra.Tr>
                        ...
                        <Chakra.Th>jenjang</Chakra.Th>
                        <Chakra.Th>fakultas</Chakra.Th>
                  ...
                  <Chakra.Tbody>
                     {data?.map((item, i) => (
                        <Chakra.Tr key={item.id}>
                           ...
                           <Chakra.Td>{item.jenjang}</Chakra.Td>
                           <Chakra.Td>{item.fakultas}</Chakra.Td>
                           <Chakra.Td>
                              <Chakra.Button
                                 onClick={() => {
            ...
            <ModalConfirm
               isOpen={isOpen}
               title={`Apakah anda ingin menghapus jurusan ${selectToDelete?.name}`}
               onClose={() => {
