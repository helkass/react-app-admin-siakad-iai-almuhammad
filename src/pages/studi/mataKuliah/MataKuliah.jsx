...
const MataKuliah = () => {
   ...
   function handleDelete() {
      deleteData(`/matkul/${selectToDelete?.id}`)
         .then((res) => {
            if (res.status === 200) {
               onClose();
               toast({
   ...
   function handleSubmit(e) {
      ...
      postData("/matkul", formData)
         .then((res) => {
            if (res.status === 200) {
               toast({
                  status: "success",
                  description: "berhasil menambahkan mata kuliah",
               });
            }
         })
         .catch((err) => {
   ...
   return (
      <Chakra.Stack>
         ...
         <Chakra.TableContainer mt="4">
               ...
               <Chakra.Table variant="striped">
                  <Chakra.Thead>
                     <Chakra.Tr>
                        ...
                        <Chakra.Th>kode</Chakra.Th>
                        <Chakra.Th>nama</Chakra.Th>
                        <Chakra.Th>sks</Chakra.Th>
                  ...
                  <Chakra.Tbody>
                     {data?.map((item, i) => (
                        <Chakra.Tr key={item.id}>
                           ...
                           <Chakra.Td>{item.kode}</Chakra.Td>
                           <Chakra.Td>{item.name}</Chakra.Td>
                           <Chakra.Td>{item.sks}</Chakra.Td>
                           <Chakra.Td>
                              <Chakra.Button
                                 onClick={() => {
            ...
            <ModalConfirm
               isOpen={isOpen}
               title={`Apakah anda ingin menghapus mata kuliah ${selectToDelete?.name}`}
               onClose={() => {
